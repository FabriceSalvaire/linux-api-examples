...
(
 ...
 uint32_t version
  ...
  if (strcmp(interface, "wl_compositor") == 0) {
    ...
    xdg_shell = wl_registry_bind(registry, name, &wl_shell_interface, 1);
  }
}
...
(
 ...
 uint32_t serial
  ...
  wl_shell_pong(xdg_shell, serial);
  printf("ping-pong\n");
}

const struct wl_shell_listener xdg_shell_listener = {
...
int main(void)
{
  ...
  wl_shell_add_listener(xdg_shell, &xdg_shell_listener, NULL);

  struct wl_surface *surface = wl_compositor_create_surface(compositor);
  struct xdg_surface *xdg_surface =
    wl_shell_get_xdg_surface(xdg_shell, surface);
